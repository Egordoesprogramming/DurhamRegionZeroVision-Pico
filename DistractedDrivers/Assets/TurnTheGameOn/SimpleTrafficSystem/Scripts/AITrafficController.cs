namespace TurnTheGameOn.SimpleTrafficSystem
{
    ...
    public class AITrafficController : MonoBehaviour
    {
        ...
        public int RegisterCarAI(AITrafficCar carAI, AITrafficWaypointRoute route)
        {
            ...
            frontHitDistanceNL.Add(frontSensorLength);
            leftHitDistanceNL.Add(sideSensorLength);
            rightHitDistanceNL.Add(sideSensorLength);
                ...
                for (int i = 0; i < carCount; i++) // operate on results
                {
                    if (isActiveNL[i] && canProcessNL[i])
                    {
                        ...
                        if (isDrivingNL[i])
                        {
                            ...
                            {
                                frontHitDistanceNL[i] = frontSensorLength;
                                frontHitNL[i] = false;
                            }
                            #endregion
                            ...
                            if (useLaneChanging)
                            {
                                if (speedNL[i] > minSpeedToChangeLanes)
                                {
                                    ...
                                    if ((forceChangeLanesNL[i] == true || frontHitNL[i] == true) && canChangeLanesNL[i] && isChangingLanesNL[i] == false)
                                    {
                                        ...
                                        {
                                            leftHitDistanceNL[i] = sideSensorLength;
                                            leftHitNL[i] = false;
                                        }
                                        #endregion
                                        ...
                                        {
                                            rightHitDistanceNL[i] = sideSensorLength;
                                            rightHitNL[i] = false;
                                        }
                                        #endregion
                        ...
                        else if (overrideInputNL[i])
                        {
                            ...
                            if (frontHitNL[i])
                            {
                                dragToAdd = Mathf.InverseLerp(0, frontSensorLength, frontHitDistanceNL[i]) * targetSpeedNL[i];
                                if (frontHitDistanceNL[i] < 1) dragToAdd = targetSpeedNL[i] * 10;

                                rigidbodyList[i].drag = minDrag + (Mathf.InverseLerp(0, frontSensorLength, frontHitDistanceNL[i]) * dragToAdd);
                                rigidbodyList[i].angularDrag = minAngularDrag + Mathf.InverseLerp(0, frontSensorLength, frontHitDistanceNL[i] * dragToAdd);
                            }
                            else
                            {
        ...
        void OnDrawGizmos()
        {
            if (STSPrefs.sensorGizmos && Application.isPlaying)
            {
                for (int i = 0; i < carTransformPositionNL.Length; i++)
                {
                    ...
                    {
                        ///// Front Sensor Gizmo
                        Gizmos.color = frontHitDistanceNL[i] == frontSensorLength ? STSPrefs.normalColor : STSPrefs.detectColor;
                        ...
                        if (STSPrefs.sideSensorGizmos)
                        {
                            ...
                            ///// Left Sensor Gizmo
                            Gizmos.color = leftHitDistanceNL[i] == sideSensorLength ? STSPrefs.normalColor : STSPrefs.detectColor;
                            offset = new Vector3(sideSensorSize.x * 2.0f, sideSensorSize.y * 2.0f, leftHitDistanceNL[i]);
                            DrawCube(leftOriginList[i] + leftDirectionList[i] * (leftHitDistanceNL[i] / 2), leftRotationList[i], offset);
                            ///// Right Sensor
                            Gizmos.color = rightHitDistanceNL[i] == sideSensorLength ? STSPrefs.normalColor : STSPrefs.detectColor;
                            offset = new Vector3(sideSensorSize.x * 2.0f, sideSensorSize.y * 2.0f, rightHitDistanceNL[i]);
